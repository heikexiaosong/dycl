...
public final class SelectorManager implements Runnable
{
    ...
    public void run()
    {
        try
        {
            while (true)
            {
                ...
                {
                    JISystem.getLogger().info("Selector manager interrupted");
                    return;
                }
        ...
        {
            cleanup();
            getLogger().error("Selector manager is unexpectedly exiting", t);
        }
    }
    ...
    private void doSelect()
    {
            ...
            if (selector.select() != 0)
            {
                ...
                while (it.hasNext())
                {
                    ...
                    catch (CancelledKeyException e)
                    {
                        ...
                        {
                            getLogger().debug("Ignoring cancelled key exception", e);
                        }
                    }
                }
            }
        ...
        {
            getLogger().error("Exception during SelectionManager select", e);
        }
    }
    ...
    private void cleanup()
    {
        for (SelectionKey key : selector.keys())
        {
            ...
            {
                getLogger().debug("Ignoring channel close exception", e);
            }
        }
        ...
        {
            getLogger().error("Ignoring selector close exception", e);
        }
    }
