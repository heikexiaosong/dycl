		...
		Thread oxidResolverThread = new Thread(new Runnable() {
			public void run() {
				...
				}catch(Exception e)
				{
					...
					{
						JISystem.getLogger().error("Oxid Resolver Thread", "run", e);
						JISystem.getLogger().warn("Oxid Resolver Thread: " +  e.getMessage() + " , on thread Id: " + Thread.currentThread().getName());
					}
				}
				finally{
					try {
		...
		Thread remUnknownThread = new Thread(remUnknownForThisListener,new Runnable() {
			public void run() {
					...
					while(true)
					{
						...
						Thread remUnknown = new Thread(remUnknownForThisListener,new Runnable() {
							public void run() {
								...
								{
									JISystem.getLogger().warn("JIComOxidRuntimeHelper RemUnknownThread (not listener)",e);
				...
				{ 
					if(JISystem.getLogger().isWarnEnabled())
					{
						JISystem.getLogger().warn("JIComOxidRuntimeHelper RemUnknownListener",e);
						JISystem.getLogger().warn("RemUnknownListener Thread: " +  e.getMessage() + " , on thread Id: " + (Thread.currentThread().getName()));
					}
					//e.printStackTrace();
				}catch(Throwable e)
				{
	...
	{ 
		...
		switch(opnum)
		{
			...
			default: //should not have arrived here.
				if (JISystem.getLogger().isWarnEnabled())
				{
					JISystem.getLogger().warn("Oxid Object: DEFAULTED !!!");
				}
...
class RemUnknownObject extends NdrObject implements IJICOMRuntimeWorker
{
	...
	static
	{
        ...
        {
            JISystem.getLogger().error("RemUnknownObject", "Static Initialiser", shouldnothappen);
        }
	}
...
//		if ("00000131-0000-0000-C000-000000000046".equalsIgnoreCase(currentIID))
		...
		else
		{
			...
			if (component == null)
			{
				JISystem.getLogger().warn("JIComOxidRuntimeHelper RemUnknownObject read(): component is null , opnum is " + opnum + " , IPID is " + ipid + " , selfIpid is " + selfIPID);
			}
			byte b[] = null;
			Object result = null;
			NetworkDataRepresentation ndr2 = new NetworkDataRepresentation();
			...
			try {
				...
				JISystem.getLogger().warn("Exception occured: " + e.getErrorCode());
		        JISystem.getLogger().error("RemUnknownObject","read",e);
			}
	...
	private NdrBuffer QueryInterface(NetworkDataRepresentation ndr)
	{
        ...
        {
			JISystem.getLogger().debug("Within RemUnknownObject: QueryInterface");
			JISystem.getLogger().debug("RemUnknownObject: [QI] Before call terminated listOfIIDsQIed are: " +  listOfIIDsQIed);
        }
		JIOrpcThis.decode(ndr);
		...
		{
			JISystem.getLogger().error("JIComOxidRuntimeHelper","QueryInterface",e);
		}
		
		if (JISystem.getLogger().isDebugEnabled())
        {
			JISystem.getLogger().debug("RemUnknownObject: [QI] IPID is " + ipid);
        }
		...
		if (JISystem.getLogger().isDebugEnabled())
        {
			JISystem.getLogger().debug("RemUnknownObject: [QI] JIJavcCoClass is " + component.getCoClassIID());
        }
		...
		for (int i = 0; i < arrayOfUUIDs.length; i++ )
		{
			...
			if (JISystem.getLogger().isDebugEnabled())
            {
				JISystem.getLogger().debug("RemUnknownObject: [QI] Array iid[" + i +  "] is " +  iid);
            }
...
//					ipid2 = GUIDUtil.guidStringFromHexString("00000000000000000000000000000000");
				...
				else
				{
					...
					if (tmpIpid == null)
					{
						if (JISystem.getLogger().isDebugEnabled())
                        {
							JISystem.getLogger().debug("RemUnknownObject: [QI] tmpIpid is null for iid " + iid);
                        }
					...
					else
					{
						if (JISystem.getLogger().isDebugEnabled())
                        {
							JISystem.getLogger().debug("RemUnknownObject: [QI] tmpIpid is NOT null for iid " + iid + " and ipid sent back is " + ipid2);
                        }
						ipid2 = tmpIpid;
					}
				}	
				...
				if (JISystem.getLogger().isDebugEnabled())
                {
					JISystem.getLogger().debug("RemUnknownObject: [QI] for which the stdObjRef is " +  objRef);
                }
		...
		if (JISystem.getLogger().isDebugEnabled())
        {
			JISystem.getLogger().debug("RemUnknownObject: [QI] After call terminated listOfIIDsQIed are: " +  listOfIIDsQIed);
        }
