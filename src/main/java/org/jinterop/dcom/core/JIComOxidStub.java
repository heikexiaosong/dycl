...
final class JIComOxidStub extends Stub{
	...
	public byte[] call(boolean isSimplePing,byte[] setId,ArrayList listOfAdds,ArrayList listOfDels, int seqNum)
	{
		...
		try {
			...
			JISystem.getLogger().error("JIComOxidStub","call",e);
		}
	...
	public void close()
	{
		...
		{
			//JISystem.getLogger().error("JIComOxidStub","close",e);
		}
	}
		...
		switch(opnum)
		{
			case 2: //complex ping
				...
				if (hresult != 0)
				{
					...
					{
						JISystem.getLogger().warn("Some error ! Complex ping failed , hresult: " + hresult);
					}
				}
				else
				{
			...
			case 1:// simple ping
				...
				if (hresult != 0)
				{
					if (JISystem.getLogger().isWarnEnabled())
					{
						JISystem.getLogger().warn("Some error ! Simple ping failed , hresult: " + hresult);
					}
				}
				else
				{
