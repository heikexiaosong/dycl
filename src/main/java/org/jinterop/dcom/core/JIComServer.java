	...
	private void initialise(JIClsid clsid,String address, JISession session) throws JIException
	{
		...
		}catch(JIException e)
		{
			if (e.getErrorCode() == 0x80040154)
			{
				...
				{
					JISystem.getLogger().warn("Got the class not registered exception , will attempt setting entries based on status flags...");
				}
				...
				if (JISystem.isAutoRegistrationSet() || clsid.isAutoRegistrationSet())
				{
					...
					try {
						...
						JISystem.getLogger().error("JIComServer","initialise",e1);
		...
		finally
		{
			...
			if (attachcomplete && serverActivation == null)
			{
				try {
					...
					if (JISystem.getLogger().isWarnEnabled())
					{
						JISystem.getLogger().warn("Unable to detach during init: " + e);
					}
				}
			}
		}
