		...
		synchronized (mutex3) {
			...
			if (holder != null)
			{
				...
				else
				{
					...
					{
						JISystem.getLogger().warn("In delIPIDReference: Could not find Original OID for this temp OID for session: " + session.getSessionIdentifier() + " , temp oid is " + oid + " , and IPID is " + IPID);
					}
					return;
				}
			...
			else
			{
				if (JISystem.getLogger().isWarnEnabled())
				{
					JISystem.getLogger().warn("In delIPIDReference: Could not find PingSetHolder for this session: " + session.getSessionIdentifier() + " , temp oid is " + oid + " , and IPID is " + IPID);
				}
			}
		}
	}
