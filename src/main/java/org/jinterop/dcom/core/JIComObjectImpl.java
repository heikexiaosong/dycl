...
final class JIComObjectImpl implements IJIComObject {
	...
	public void addRef() throws JIException
	{
        ...
        {
			JISystem.getLogger().warn("addRef: Adding 5 references for " + ptr.getIPID() + " session: " + session.getSessionIdentifier());
        }
...
//		session.getStub2().addRef_ReleaseRef(obj);
	...
	public void release() throws JIException
	{
		...
		if (JISystem.getLogger().isInfoEnabled())
        {
			JISystem.getLogger().warn("RELEASE called directly ! removing 5 references for " + ptr.getIPID()+ " session: " + session.getSessionIdentifier());
