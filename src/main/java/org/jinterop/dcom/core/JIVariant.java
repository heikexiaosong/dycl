...
//		supportedTypes.put(JIDispatchImpl.class,new Integer(VT_DISPATCH));
	...
	public static JIVariant makeVariant(Object o, boolean isByRef)
	{
		...
		}catch(Exception e)
		{
			...
			{
				JISystem.getLogger().warn("Could not create Variant for " + o + " , isByRef " + isByRef);
			}
		}
...
//			}
			...
			{
				JISystem.getLogger().debug("Variant length is " + length + " , value " + value + " , variant type" + type);
			}
...
//	}
	...
	private int getVarType(Class c, Object obj)
	{
		...
		if (c != null)
		{
			...
			{
				JISystem.getLogger().warn("In getVarType: Unsupported Type found ! " + c + " , please add this to the supportedType map ! ");
	...
	private static JIStruct getDecodedValueAsArray(NetworkDataRepresentation ndr,List defferedPointers, int type,boolean isByRef,Map additionalData, int FLAG)
	{
		...
		try {
			...
			if (c == null)
			{
				if (JISystem.getLogger().isWarnEnabled())
				{
					JISystem.getLogger().warn("From JIVariant: while decoding an Array, type " + type + " , was not found in supportedTypes_classes map , hence using JIVariant instead...");
				}
