...
//	   	objRef.oidString = byteArrayOutputStream.toString();
		...
		try {
			...
			JISystem.getLogger().error("JIStdObjRef","decode",e);
		}
...
//			ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
//		   	jcifs.util.Hexdump.hexdump(new PrintStream(byteArrayOutputStream), objRef.oid, 0, objRef.oid.length);
//		   	JISystem.getLogger().warn("Decode of StdObjref Adding references for " + objRef.ipidOfthisObjectRef + " , num references recieved from COM server: " + objRef.publicRefs + " , the OID is " + byteArrayOutputStream.toString());
...
//        }
	...
	public void encode(NetworkDataRepresentation ndr)
	{
		...
		try {
			...
			JISystem.getLogger().error("JIStdObjRef","encode",e);
		}
	}
