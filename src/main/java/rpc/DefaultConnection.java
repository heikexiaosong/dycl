...
public class DefaultConnection implements Connection {
                ...
                try {
                    ...
                    if (currentFragment.getFlag(
                        ...
                        try {
							...
							if (logger.isDebugEnabled())
                            {
                            	logger.debug("[Fragmented Packet] [" + i++ + "] recieved , fragment decomposition is below:- ");
                            }
    ...
    protected void transmitFragment(ConnectionOrientedPdu fragment,
        ...
        if (logger.isDebugEnabled())
        {
        	...
        	Hexdump.hexdump(new PrintStream(byteArrayOutputStream), transmitBuffer.getBuffer(), 0, transmitBuffer.length);
        	logger.debug("[TRANSMIT BUFFER]:-\n" + byteArrayOutputStream.toString());
        }
...
//				if (isValidType(type))
    	...
    	if (read)
    	{
    		...
    		if (logger.isDebugEnabled())
    	    {
	    		logger.debug("[RECIEVER BUFFER] Full packet is dumped below...");
    	        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
    	        Hexdump.hexdump(new PrintStream(byteArrayOutputStream), receiveBuffer.getBuffer(), 0, receiveBuffer.length);
    	        logger.debug("\n" + byteArrayOutputStream.toString());
...
//			fragmentLength = receiveBuffer.dec_ndr_short(); 
			...
			if (logger.isDebugEnabled())
    	    {
    	        ...
    	        ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
    	        Hexdump.hexdump(new PrintStream(byteArrayOutputStream), bufferToBeUsed.getBuffer(), 0, bufferToBeUsed.length);
    	        logger.debug("\n" + byteArrayOutputStream.toString());
    	        logger.debug("\n*********************************************************************************");
    	    }
