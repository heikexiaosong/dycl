...
public class RequestCoPdu extends ConnectionOrientedPdu
    ...
    private static final Logger logger = LoggerFactory.getLogger("org.jinterop"); // LoggerFactory.getLogger("org.jinterop");
    ...
    public Iterator fragment(int size) {
        ...
        {
        	logger.debug("In fragment of RequestCoPdu, this packet will be fragmented while sending...\n");
        }
...
//            }
            if (logger.isDebugEnabled())
            {
            	logger.debug("In FragementIterator:next(): callIdCounter is " + callId + " ,  for thread: " + Thread.currentThread());
            }
            return fragment;
        }
